...
sub Register {
	...
	if ( $args{'LOGNAME'} eq 'ALL' ) {
		...
		# with an array, let them put it in twice!
#		# Put it in the hash!
...
#			}
#		} else {
		push(@{$_[HEAP]->{'ALLLOGS'}->{ $args{'SESSION'} } }, $args{'EVENT'} );
#		}
	} else {
...
#			}
#		} else {
		push( @{$_[HEAP]->{'LOGS'}->{ $args{'LOGNAME'} }->{ $args{'SESSION'} } }, $args{'EVENT'} );
#		}
	}
...
sub UnRegister {
	...
	if ( $args{'LOGNAME'} eq 'ALL' ) {
		...
		if ( exists $_[HEAP]->{'ALLLOGS'}->{ $args{'SESSION'} } ) {
			...
			foreach $evnt ( @{ $_[HEAP]->{'ALLLOGS'}->{ $args{'SESSION'} } } ) {
				if (  $evnt  eq $args{'EVENT'} ) {
					# Found a match, delete it!
					splice( @{ $_[HEAP]->{'ALLLOGS'}->{ $args{'SESSION'} } },
						$index, 1 );
				}
				$index++;
			}
		...
		if ( exists $_[HEAP]->{'LOGS'}->{ $args{'LOGNAME'} } ) {
			...
			if ( exists $_[HEAP]->{'LOGS'}->{ $args{'LOGNAME'} }->{ $args{'SESSION'} } ) {
				# Scan for the proper event!
				$index = 0;
				foreach $evnt ( @{ $_[HEAP]->{'LOGS'}->{ $args{'LOGNAME'} }->{ $args{'SESSION'} } } ) {
					if ( $evnt eq $args{'EVENT'} ) {
						# Found a match, delete it!
#						delete $_[HEAP]->{'LOGS'}->{ $args{'LOGNAME'} }->{ $args{'SESSION'} }->{ $evnt };
						splice( @{ $_[HEAP]->{'LOGS'}->{ $args{'LOGNAME'} }->{ $args{'SESSION'} } },
							$index, 1 );
						# Return success
					}
					$index++;
	...
	# Hacker: I didn't fix this for arrays, since we can't return in case it IS registered more than once.
	# Found nothing...
	return undef;
}
...
sub UnRegisterSession {
	...
	foreach my $logname ( keys %{ $_[HEAP]->{'LOGS'} } ) {
		# Another loop!
		foreach $session ( keys %{ $_[HEAP]->{'LOGS'}->{ $logname } } ) {
			# Check if they match
	...
	foreach $session ( keys %{ $_[HEAP]->{'ALLLOGS'} } ) {
		# Check if they match
		if ( $session eq $TargetSession ) {
			# Remove this!
...
sub Log {
	...
	# Figure out the time, using same time for each subscriber.
	my $time;
...
#				foreach my $event ( keys %{ $_[HEAP]->{'LOGS'}->{ $logname }->{ $TargetSession } } ) {
				foreach $event ( @{ $_[HEAP]->{'LOGS'}->{ $logname }->{ $TargetSession } } ) {
		...
		if ( keys %{ $_[HEAP]->{'ALLLOGS'} } > 0 ) {
			# Oh boy, send the log!
			foreach $TargetSession ( keys %{ $_[HEAP]->{'ALLLOGS'} } ) {
				# Find out if this session exists
				if ( ! $_[KERNEL]->ID_id_to_session( $TargetSession ) ) {
					# Argh...
					if ( DEBUG ) {
...
#					foreach my $event ( keys %{ $_[HEAP]->{'ALLLOGS'}->{ $TargetSession } } ) {
					foreach $event ( @{ $_[HEAP]->{'ALLLOGS'}->{ $TargetSession } } ) {
